...
export default function ResultsPage() {
  ...
  const handleGenerateQuiz = async () => {
    ...
    try {
      ...
      if (resData.status === "success") {
        // Boom! Redirect to the new quiz page
      ...
      alert("Connection error generating quiz. Ensure FastAPI is running.");
      setIsGeneratingQuiz(false);
    }
  };
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { role: 'assistant', content: "Connection error. Ensure FastAPI is running on port 8000." }]);
    } finally {
            ...
            {resourceLinks.length > 0 ? (
              <div className="space-y-3">
                {resourceLinks.map((link: any, index: number) => {
                  ...
                  return (
                    <a key={index} href={cleanUrl} target="_blank" rel="noopener noreferrer" className="block p-4 bg-slate-50 hover:bg-indigo-50 border border-slate-100 hover:border-indigo-200 rounded-2xl transition-all group">
                      <p className="text-sm font-semibold text-slate-700 group-hover:text-indigo-700 line-clamp-2">{link.title}</p>
                      <span className="text-xs text-indigo-500 mt-2 inline-flex items-center gap-1 font-medium">Read more <ExternalLink className="w-3 h-3" /></span>
                    </a>
                  )
                })}
              </div>
